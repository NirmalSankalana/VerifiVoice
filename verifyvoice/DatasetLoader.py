...
class train_dataset_loader(Dataset):
    def __init__(self, train_list, augment, musan_path, rir_path, max_frames, train_path, train_list_percentage, **kwargs):
        ...
        self.train_list_percentage = train_list_percentage
        ...
        num_samples = int(len(lines) * self.train_list_percentage)
        print(f"Number of data in list: {len(lines)}")
        print(f"Number of training samples : {num_samples}")
        print(f"percentage = {num_samples/len(lines)*100:.2f} %")


        for lidx, line in enumerate(lines[:num_samples]):
