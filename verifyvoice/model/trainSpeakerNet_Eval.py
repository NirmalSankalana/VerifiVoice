...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.eval == True:
        scorefile_score = open(args.result_save_path + "/Eval_scores_mean_O_All.txt", "a");
        print('Test list', args.test_list)

        for i in range(1, 12):
            print(f"Model {modelfiles[-i]} loaded from previous state!");
            trainer.loadParameters(modelfiles[-i]);
            # trainer.loadParameters(modelfiles[0]);           

            # sc, lab, _,sc1,sc2 = trainer.evaluateFromList_1utterance(**vars(args))226517
            ...
            if args.gpu == 0:
                ...
                scorefile_score.write(
                    "Epoch {}, VEER {:2.4f}, VEER_S1 {:2.4f}, VEER_S2 {:2.4f}, MinDCF05 {:2.5f}, MinDCF01 {:2.5f}\n".format(
                        modelfiles[-i], result[1], result1[1], result2[1], mindcf, mindcf_1));
...
def main():
    ...
    args.model_save_path = f"{args.save_path}/model"
    args.result_save_path = f"{args.save_path}/result"
    args.feat_save_path = ""
