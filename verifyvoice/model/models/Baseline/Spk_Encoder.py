...
class spk_extractor(nn.Module):
    ...
    def forward(self,wav_and_flag):
        ...
        # print(f"torch.stack(layer_reps) = {torch.stack(layer_reps).shape}")
        # print(f"torch.stack(layer_reps).transpose(0,-1) = {torch.stack(layer_reps).transpose(0,-1).shape}")
        # print(f"torch.stack(layer_reps).transpose(0,1) = {torch.stack(layer_reps).transpose(0,-1).shape}")
